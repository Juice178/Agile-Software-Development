from src.ch5.generate_primes_v1 import GeneratePrimes
...
class TestGeneratePrimes:
    ...
    def test_null_array(self) -> None:
        null_array: list[int] = GeneratePrimes.generate_primes(0)
        assert len(null_array) == 0, f"Array size is {len(null_array)}"

    def test_min_array(self) -> None:
        min_array: list[int] = GeneratePrimes.generate_primes(2)
    ...
    def test_three_array(self) -> None:
        three_array: list[int] = GeneratePrimes.generate_primes(3)
    ...
    def test_cent_array(self) -> None:
        cent_array: list[int] = GeneratePrimes.generate_primes(100)
